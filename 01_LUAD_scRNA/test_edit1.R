...
FeaturePlot(prox_pnec_basal, reduction = "umap", c("ENSMUSG00000024653","ENSMUSG00000064057")) #club (scgb1a1)
...
DimPlot(prox_pnec_basal, label = TRUE)
DimPlot(prox_epi_seurat, label = TRUE)

plot <-FeaturePlot(prox_pnec_basal, reduction = "umap", c("ENSMUSG00000064057")) #
plot
select_cluster <- CellSelector(plot = plot)
Idents(prox_pnec_basal, cells = select_cluster) <- "Clara"
table(prox_pnec_basal@active.ident)

#prox_pnec_basal <- RenameIdents(object = prox_pnec_basal, "Ciliated"="5")
#prox_pnec_basal <- RenameIdents(object = prox_pnec_basal, "Clara"="rm")
...
test6 <- subset(prox_pnec_basal, idents = c("PNEC", "Basal","Clara","Ciliated")) #PNEC = 36/ Basal = 228/ Ciliated = 127/ Clara =105
test7 <- subset(prox_epi_seurat, idents = c("PNEC", "Basal","Clara","Ciliated")) #PNEC = 70/ Basal = 239/ Ciliated = 124/ Clara = 101
...
#test5 = test1 + test2 + test4 (batch balancing with basal, PNEC_no Clara & Ciliated)
...
test8 <- JackStraw(test8, dims = 50)
test8 <- ScoreJackStraw(test8, dims = 1:50)
JackStrawPlot(test8, dims = 1:50)
...
pdf("test8_edit1.pdf", 7,5)
for(k in c(4:6)){
  for(trim in c(25,27,30,33,35,37,40,43,45)){
    print(
...
FeaturePlot(test8, c("ENSMUSG00000023951"), reduction = "umap") #AT1 "ENSMUSG00000059325","ENSMUSG00000028583","ENSMUSG00000023039","ENSMUSG00000023951"
FeaturePlot(test8, c("ENSMUSG00000041247"), reduction = "umap") #AT2 "ENSMUSG00000029375","ENSMUSG00000037071","ENSMUSG00000042784"
FeaturePlot(test8, c("ENSMUSG00000038791","ENSMUSG00000031722"), reduction = "umap", max.cutoff = 8) #Clara "ENSMUSG00000022595"
FeaturePlot(test8, c("ENSMUSG00000027676","ENSMUSG00000040703"), reduction = "umap") #Ciliated "ENSMUSG00000040703", "ENSMUSG00000024653" ,"ENSMUSG00000027676"
FeaturePlot(test8, reduction = "umap", c("ENSMUSG00000021194","ENSMUSG00000030669", "ENSMUSG00000020052"), max.cutoff = 5) #PNEC
FeaturePlot(test8, reduction = "umap", c("ENSMUSG00000028435","ENSMUSG00000022676"), max.cutoff = 5) #BASAL

test8 <- test8 %>% RunBBKNN(dims.use = 1:20,
                            neighbors_within_batch = 4,
                            trim = 43,
                            batch.key = "batch",
                            python.path = "/home/users/yunah1029/anaconda3/bin/python")
DimPlot(test8, reduction = "bbknn", group.by = "batch") + ggtitle(paste0("bbknn1; neighbors=",4,",trim=",43,",add_PNEC&Basal"))
####4, 43 찾음 !! (2020.01.08) /4, 30/ 
...
##Gene set 결정
test8_markers <- FindAllMarkers(test8, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
table(test8_markers$cluster)
test8_markers <- test8_markers %>% group_by(cluster) %>% top_n(n = 100, wt = avg_logFC) #700
test8_markers_rm_nonepi<-setdiff(test8_markers$gene, nonepi_marker_dt$gene_name) #658

DoHeatmap(test8, features = test8_markers$gene)
DoHeatmap(test8, features = test8_markers_rm_nonepi)

###epi marker + lung subtype markers = final_genes ->738
test8_final <- union(test8_markers_rm_nonepi, cur_markers$gene_name ) 
test8_final<-setdiff(test8_final, luad_genes) #luad_gene들은 제거/ 그대로 유지.
length(test8_final)

###+final gene 수정; 738 (그대로 유지)
### cell cycle related gene 21개 + luad related gene 1개 제거
test8_final <- setdiff(test8_final, c("ENSG00000118971","ENSG00000039068","ENSG00000198668","ENSG00000135446","ENSG00000105976","ENSG00000171848","ENSG00000073350","ENSG00000100714","ENSG00000150753","ENSG00000111057","ENSG00000148834","ENSG00000144381","ENSG00000155368","ENSG00000173207","ENSG00000010278","ENSG00000075131","ENSG00000129757","ENSG00000105968","ENSG00000188486","ENSG00000160752","ENSG00000110092","ENSG00000147883"))
DoHeatmap(test8, features = test8_final$mouse_ensembl)

###Final_gene to human ensembl; 736
test8_final_df <- test8_final%>%as.data.frame()
colnames(test8_final_df) <- "epi_mouse"
test8_final<-merge(test8_final_df, ensembl_mouse_human, by.x = "epi_mouse", by.y = "mouse_ensembl")
table(duplicated(test8_final$human_ensembl))
test8_final<- test8_final[!duplicated(test8_final[2]),]
head(test8_final)
names(test8_final)[1]<-"mouse_ensembl"
dim(test8_final) #736gene


test8_final_markers <- test8_markers_rm_nonepi %>% as.data.frame()
colnames(test8_final_markers) <- "epi_mouse"
test8_final_markers<-merge(test8_final_markers, ensembl_mouse_human, by.x = "epi_mouse", by.y = "mouse_ensembl")
table(duplicated(test8_final_markers$human_ensembl))
test8_final_markers<- test8_final_markers[!duplicated(test8_final_markers[2]),]
head(test8_final_markers)
names(test8_final_markers)[1]<-"mouse_ensembl"
dim(test8_final_markers) #736gene


##single cell data processing -> with 700 genes (expression data)   #gene set; test8_final -> test8_final_markers(539genes)
mouse_epi_traj<-test8@assays$RNA@data %>%as.data.frame() %>%rownames_to_column() # cell; 6476 / gene; 11528
names(mouse_epi_traj)[1] <- c("ensembl")
mouse_epi_700 <- merge(mouse_epi_traj, test8_final, by.x = "ensembl", by.y = "mouse_ensembl") #734
...
a<-Embeddings(test8,reduction = "bbknn") %>% as.data.frame()
