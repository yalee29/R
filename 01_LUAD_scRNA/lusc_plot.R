...
cor_single_bulk_lusc_sort <- cor_single_bulk_lusc_sort[,which(cor_max_lusc > 0.45)]
...
for (x in 1:nrow(cor_single_bulk_lusc_sort_rank)) {
  tmp_original_matched <- a[cor_single_bulk_lusc_sort_rank[x,],]
  tmp_original_matched$group <- "LUSC:Origianl genes_Median"
  tmp_original_matched$cancer_sample <- rownames(cor_single_bulk_lusc_sort_rank)[x]
  ...
  tmp_original_avg$cell_id <- rownames(cor_single_bulk_lusc_sort_rank)[x]
  tmp_original_avg$cancer_sample <- rownames(cor_single_bulk_lusc_sort_rank)[x]
...
repeat{
  random_600<-sample(rownames(single_bulk_lusc), 591, replace = TRUE)
  single_bulk_random <- single_bulk_lusc %>% filter(rownames(single_bulk_lusc) %in% random_600) 
...
pdf("perturbation_sampled_genes2_v1_dots.pdf",8,5)
...
pdf("perturbation(LUSC)_sampled_genes2_v3_chull_medians_0.45.pdf",8,5)
...
FeaturePlot(E_seurat1_ad_epi_bb, c("ENSMUSG00000056370","ENSMUSG00000038791","ENSMUSG00000028583","ENSMUSG00000059325"), reduction = "bbknn")

#heatmap version 2
library(NMF) #heatmap version2
tmp <- single_bulk 
tmp <- tmp %>% as.data.frame 
tmp_annotation <- E_seurat1_ad_epi_bb@meta.data %>% rownames_to_column() %>% select(rowname, batch) %>% arrange(batch) %>% column_to_rownames('rowname')
aheatmap(tmp, scale = "row", annCol = tmp_annotation, Rowv = NA, Colv = NA, height = 30, cexRow = 10)
